...
const handleSubmit = (e) =>{
    e.preventDefault();
    db.collection('Registered_Employees').doc(user.email).set({
        name : empName,
        number : number,
        email : user.email,
        age : age,
...
const notSignedIn = () =>{
  ...
  return (
    ...
    <div className='first_img_div'>
      ...
      <h1>SERVICE PROVIDER'S <br/> REGISTRATION PAGE!!</h1>
      <p>Do you want to work FreeLance?</p>
    ...
    <input placeholder='Age' type="required" onChange={e => setAge(e.target.value)}/>
     ...
     <div className='input_div'>
    <input input="radio" type="required" placeholder='What is your Specialization?' onChange={e => setSpl(e.target.value)} />
    </div>
    <div className='input_div'>
